...
Its implementation can be found in the `clip-team-branch` branch within `fellowship/price-discovery` GitHub repository. There is an `Agents.ipynb` notebook with code implementations.
...
- The RAG model encodes the query into text and image embeddings, which are then compared to a vector database of information. The query can be passed into the vector database in any one of the two ways:
    ...
    - Secondly, for a more precise result, a new description is generated from the text query and image query analysis using a VLLM. In this case, Gemini pro-vision was used.
